...
void DataWriter::loadSpriteOntoFPGA(int address, int size)
{
	...
	{	 
        ...
        GPIO.out |= writeToGPIO(data); 
		vTaskDelay(0.1/portTICK_PERIOD_MS);  

        GPIO.out |= 0b100;                  //write enable high
		vTaskDelay(1/portTICK_PERIOD_MS);   //wait one MS  
...
int DataWriter::writeToGPIO(int *aValue)
{
    int output = 0;
    int temp = 0;
